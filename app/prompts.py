...
대화의 흐름순서로 진료상담 보고서를 작성하고, Tool과 관련된 Message는 보고서에 포함되지 않습니다. 상담의의 질문(AI Message)과 그에 대한 환자의 답변(Human Message)을 상세하게 보고서에 포함합니다. 진단 및 의학과 관련없는 내용은 보고서에 절대로 포함하지 않습니다.
...
| 상태 | 판단 조건 | 행동 |
|------|----------|------|
| A: 최초 호출 | expert 의견이 없음 AND mid_term_diagnosis_summary 없음 | expert1, expert2, expert3 동시 요청 (fan-out) |
...
| F: 2차+ 라운드 최초 호출 | mid_term_diagnosis_summary 있음 AND expert 의견 없음 (supervisor_messages 비어있음) | expert1, expert2, expert3 동시 요청 (fan-out). instruction에 이전 분석 내용 + 새 정보 수집 사실 + 진단 갱신 요청 명시 |
...
4. **2차+ 라운드 판단**:
   - supervisor_messages가 비어있고 mid_term_diagnosis_summary가 존재하면 → State F (2차+ 라운드 최초 호출)
   - supervisor_messages가 비어있고 mid_term_diagnosis_summary가 없으면 → State A (최초 호출)
